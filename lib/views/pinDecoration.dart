...
class PinInputTextField extends StatefulWidget {
  ...
  /// Hint length must equal to the [pinLength].
        ...
        assert(decoration is BoxTightDecoration || (decoration is UnderlineDecoration && pinLength - 1 == (decoration.gapSpaces?.length ?? (pinLength - 1))) || (decoration is BoxLooseDecoration && pinLength - 1 == (decoration.gapSpaces?.length ?? (pinLength - 1)))),
        inputFormatters = inputFormatter == null ? <TextInputFormatter>[WhitelistingTextInputFormatter.digitsOnly, LengthLimitingTextInputFormatter(pinLength)] : inputFormatter
...
//          offset: _effectiveController.text.runes.length);
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      ...
      foregroundPainter: _PinPaint(text: _text ?? _text.trim(), pinLength: widget.pinLength, decoration: widget.decoration, themeData: Theme.of(context)),
      child: TextField(
...
class _PinPaint extends CustomPainter {
  ...
  _PinPaint({
    ...
    errorTextStyle: decoration.errorTextStyle ?? themeData.textTheme.caption.copyWith(color: themeData.errorColor),
    hintTextStyle: decoration.hintTextStyle ?? themeData.textTheme.headline.copyWith(color: themeData.hintColor),
  );
  ...
  _drawBoxLoose(Canvas canvas, Size size) {
    ...
    text.runes.forEach((rune) {
      ...
      startX = singleWidth * index + singleWidth / 2 - textPainter.width / 2 + _sumList(gapSpaces.take(index)) + dr.strokeWidth * index * 2 + dr.strokeWidth;
      textPainter.paint(canvas, Offset(startX, startY));
      index++;
    });
